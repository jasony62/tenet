...
import io.ctsi.tenet.kafka.connect.data.error.SchemaBuilderException;

import java.util.*;

public class SchemaBuilder implements io.ctsi.tenet.kafka.connect.data.Schema {
    ...
    private io.ctsi.tenet.kafka.connect.data.Schema keySchema = null;
    private io.ctsi.tenet.kafka.connect.data.Schema valueSchema = null;
     ...
     * @return the SchemaBuilder
     */
    public SchemaBuilder field(String fieldName, io.ctsi.tenet.kafka.connect.data.Schema fieldSchema) {
        if (type != Type.STRUCT)
     ...
     */
    public static SchemaBuilder array(io.ctsi.tenet.kafka.connect.data.Schema valueSchema) {
        if (null == valueSchema)
     ...
     */
    public static SchemaBuilder map(io.ctsi.tenet.kafka.connect.data.Schema keySchema, io.ctsi.tenet.kafka.connect.data.Schema valueSchema) {
        if (null == keySchema)
    ...
    static SchemaBuilder mapWithNullKeys(io.ctsi.tenet.kafka.connect.data.Schema valueSchema) {
        SchemaBuilder result = new SchemaBuilder(Type.MAP);
        result.valueSchema = valueSchema;
        return result;
    }

    static SchemaBuilder mapWithNullValues(io.ctsi.tenet.kafka.connect.data.Schema keySchema) {
        SchemaBuilder result = new SchemaBuilder(Type.MAP);
    ...
    @Override
    public io.ctsi.tenet.kafka.connect.data.Schema keySchema() {
        return keySchema;
    }

    @Override
    public io.ctsi.tenet.kafka.connect.data.Schema valueSchema() {
        return valueSchema;
    }
     ...
     * @return the {@link io.ctsi.tenet.kafka.connect.data.Schema}
     */
    public io.ctsi.tenet.kafka.connect.data.Schema build() {
    ...
    /**
     * Return a concrete instance of the {@link io.ctsi.tenet.kafka.connect.data.Schema} specified by this builder
     * @return the {@link io.ctsi.tenet.kafka.connect.data.Schema}
     */
    @Override
