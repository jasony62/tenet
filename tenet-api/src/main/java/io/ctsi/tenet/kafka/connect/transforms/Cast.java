...
import io.ctsi.tenet.kafka.connect.transforms.util.Requirements;
...
public abstract class Cast<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    private static Object encodeLogicalType(Schema schema, Object value) {
        switch (schema.name()) {
            case io.ctsi.tenet.kafka.connect.data.Date.LOGICAL_NAME:
                return io.ctsi.tenet.kafka.connect.data.Date.fromLogical(schema, (Date) value);
