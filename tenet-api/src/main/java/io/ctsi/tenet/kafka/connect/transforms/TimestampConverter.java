...
import io.ctsi.tenet.kafka.connect.transforms.util.SchemaUtil;
import io.ctsi.tenet.kafka.connect.transforms.util.SimpleConfig;
...
import static io.ctsi.tenet.kafka.connect.transforms.util.Requirements.requireMap;
import static io.ctsi.tenet.kafka.connect.transforms.util.Requirements.requireStructOrNull;
...
public abstract class TimestampConverter<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    public static final Schema OPTIONAL_DATE_SCHEMA = io.ctsi.tenet.kafka.connect.data.Date.builder().optional().schema();
    ...
    static {
        ...
        TRANSLATORS.put(TYPE_DATE, new TimestampTranslator() {
            ...
            @Override
            public Schema typeSchema(boolean isOptional) {
                return isOptional ? OPTIONAL_DATE_SCHEMA : io.ctsi.tenet.kafka.connect.data.Date.SCHEMA;
            }
    ...
    private String timestampTypeFromSchema(Schema schema) {
        ...
        } else if (io.ctsi.tenet.kafka.connect.data.Date.LOGICAL_NAME.equals(schema.name())) {
            return TYPE_DATE;
