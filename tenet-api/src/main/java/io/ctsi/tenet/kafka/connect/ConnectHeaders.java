...
public class ConnectHeaders implements Headers {
    ...
    public Headers addStruct(String key, Struct value) {
        ...
        checkSchemaType(value.schema(), Schema.Type.STRUCT);
    ...
    public Headers addDate(String key, Date value) {
        if (value != null) {
            ...
            io.ctsi.tenet.kafka.connect.data.Date.fromLogical(io.ctsi.tenet.kafka.connect.data.Date.SCHEMA, value);
        }
        return addWithoutValidating(key, value, io.ctsi.tenet.kafka.connect.data.Date.SCHEMA);
    }
                ...
                switch (schema.type()) {
                    ...
                    case INT32:
                        if (value instanceof Integer)
                            return;
                        if (value instanceof Date && io.ctsi.tenet.kafka.connect.data.Date.LOGICAL_NAME.equals(schema.name()))
                            return;
                        if (value instanceof Date && Time.LOGICAL_NAME.equals(schema.name()))
                            return;
                        break;
                    case INT64:
                        if (value instanceof Long)
                            return;
                        if (value instanceof Date && Timestamp.LOGICAL_NAME.equals(schema.name()))
                            return;
                        break;
                    case FLOAT32:
