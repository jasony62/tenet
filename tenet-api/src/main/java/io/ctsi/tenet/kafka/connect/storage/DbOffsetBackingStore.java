...
import io.ctsi.tenet.kafka.connect.util.SafeObjectInputStream;
...
//TODO
public class DbOffsetBackingStore extends io.ctsi.tenet.kafka.connect.storage.MemoryOffsetBackingStore {
    private static final Logger log = LoggerFactory.getLogger(DbOffsetBackingStore.class);
    ...
    private void load() {
        try (SafeObjectInputStream is = new SafeObjectInputStream(Files.newInputStream(new File("").toPath()))) {
    ...
    @Override
    protected void save() {
        try (ObjectOutputStream os = new ObjectOutputStream(Files.newOutputStream(new File("").toPath()))) {
