...
import io.ctsi.tenet.kafka.mongodb.sink.converter.SinkConverter;
...
import static io.ctsi.tenet.kafka.mongodb.sink.writemodel.strategy.WriteModelStrategyHelper.createValueWriteModel;
...
public class MongoSinkTask extends SinkTask {
    ...
    private String appendSuffix(String list, String suffix) {
        //list =  list.stream().map(str -> str+suffix ).collect(Collectors.toList());
        StringBuilder builder = new StringBuilder();
        Arrays.stream(list.split(",")).forEach(e->{
            builder.append(e+suffix);
        });
...
//    }
    ...
    public void start(final Map<String, String> props) {
        ...
        try {
            props.put(TOPICS_CONFIG, appendSuffix(props.get(TOPICS_CONFIG), props.get("topic_suffix")));
                                                    ...
                                                    (t) ->
                                                            new AtomicInteger(
                                                                    sinkConfig
                                                                            ...
                                                                            .getInt(MongoSinkTopicConfig.MAX_NUM_RETRIES_CONFIG)))));
        } catch (Exception e) {
     ...
     * RetriableException} to indicate that the framework should
    ...
    private MongoClient getMongoClient() {
        if (mongoClient == null) {
            mongoClient =
                    MongoClients.create(
                            sinkConfig.getConnectionString(), ConfigHelper.getMongoDriverInformation(CONNECTOR_TYPE));
        }
        return mongoClient;
    }
    ...
    private AtomicInteger getRemainingRetriesForTopic(final String topic) {
        if (!remainingRetriesTopicMap.containsKey(topic)) {
            remainingRetriesTopicMap.put(
                    topic,
                    new AtomicInteger(
                            sinkConfig.getMongoSinkTopicConfig(topic).getInt(MongoSinkTopicConfig.MAX_NUM_RETRIES_CONFIG)));
        }
    ...
    private void resetRemainingRetriesForTopic(final MongoSinkTopicConfig topicConfig) {
        getRemainingRetriesForTopic(topicConfig.getTopic())
                .set(topicConfig.getInt(MongoSinkTopicConfig.MAX_NUM_RETRIES_CONFIG));
    }
    ...
    private void handleMongoException(final MongoSinkTopicConfig config, final MongoException e) {
        if (getRemainingRetriesForTopic(config.getTopic()).decrementAndGet() <= 0) {
            ...
            Integer deferRetryMs = config.getInt(MongoSinkTopicConfig.RETRIES_DEFER_TIMEOUT_CONFIG);
    ...
    Map<String, RecordBatches> createSinkRecordBatchesPerTopic(final Collection<SinkRecord> records) {
        ...
        records.forEach(
                r -> {
                    ...
                    if (batches == null) {
                        int maxBatchSize =
                                sinkConfig.getMongoSinkTopicConfig(r.topic()).getInt(MongoSinkTopicConfig.MAX_BATCH_SIZE_CONFIG);
                        LOGGER.debug(
