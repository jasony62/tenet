...
import io.ctsi.tenet.kafka.mongodb.sink.processor.field.projection.AllowListProjector;
...
public class AllowListValueProjector extends AllowListProjector {
  ...
  public AllowListValueProjector(final MongoSinkTopicConfig config) {
    this(config, config.getString(MongoSinkTopicConfig.VALUE_PROJECTION_LIST_CONFIG));
  }

  public AllowListValueProjector(final MongoSinkTopicConfig config, final String fieldList) {
    super(config, buildProjectionList(MongoSinkTopicConfig.FieldProjectionType.ALLOWLIST, fieldList), SinkDocumentField.VALUE);
  }
}
