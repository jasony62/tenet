...
public class MongoSinkConfig extends AbstractConfig {
    ...
    public static final String CONNECTION_URI_CONFIG = "connection.url";
...
//                        : Optional.of(unmodifiableList(getList(TOPICS_CONFIG)))
        ...
        if (topicsRegex.isPresent()) {
            ...
            originals.keySet().stream()
                    .filter(k -> k.startsWith(MongoSinkTopicConfig.TOPIC_OVERRIDE_PREFIX))
                    .forEach(
                            k -> {
                                String topic = k.substring(MongoSinkTopicConfig.TOPIC_OVERRIDE_PREFIX.length()).split("\\.")[0];
    ...
    public MongoSinkTopicConfig getMongoSinkTopicConfig(final String topic) {
        // String newTopic = topic.replaceAll(TOPIC_REGEX,"$1");
        if (!topicSinkConnectorConfigMap.containsKey(topic)) {
            topics.ifPresent(
                    topicsList -> {
    ...
    private static ConfigDef createConfigDef() {
        ...
        configDef.define(
                ...
                Validators.errorCheckingValueValidator("A valid connection string", ConnectionString::new),
                ConfigDef.Importance.HIGH,
                CONNECTION_URI_DOC,
                group,
                ++orderInGroup,
