...
public class DocumentIdAdder extends PostProcessor {
  ...
  public DocumentIdAdder(final MongoSinkTopicConfig config) {
    ...
    this.overwriteExistingIdValues =
        config.getBoolean(MongoSinkTopicConfig.DOCUMENT_ID_STRATEGY_OVERWRITE_EXISTING_CONFIG);
  }
  ...
  public void process(final SinkDocument doc, final SinkRecord orig) {
    doc.getValueDoc()
        .ifPresent(
            vd -> {
              if (shouldAppend(vd)) {
                vd.append(MongoSinkTopicConfig.ID_FIELD, idStrategy.generateId(doc, orig));
              }
            });
  }
  ...
  private boolean shouldAppend(final BsonDocument doc) {
    return !doc.containsKey(MongoSinkTopicConfig.ID_FIELD) || overwriteExistingIdValues;
  }
}
